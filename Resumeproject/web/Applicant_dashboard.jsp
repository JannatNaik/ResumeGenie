...
<div class="profile-container">
    <div class="profile-card">
        <%
    // Optional: Fetch suggestions from DB using uid
    String suggestions = "";
    int uid = (Integer) session.getAttribute("uid");
    try {
        Class.forName("com.mysql.cj.jdbc.Driver");
        Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/resume_genie", "root", "yourpassword");
        PreparedStatement stmt = conn.prepareStatement("SELECT suggestions FROM ai_resume_analysis WHERE uid = ?");
        stmt.setInt(1, uid);
...
<% if (suggestions != null && !suggestions.trim().isEmpty()) { %>
    <div style="margin-top: 20px; padding: 15px; border: 2px dashed #2c7be5; border-radius: 10px; background: #eef7ff;">
        <h3 style="margin: 0 0 10px;">Suggestions:</h3>
        <p style="white-space: pre-wrap; font-size: 15px;"><%= suggestions %></p>
    </div>
<% } %>
        ...
        <div class="buttons">
            ...
            <form method="post" action="UploadResumeServlet" enctype="multipart/form-data" style="display: flex; flex-direction: column; align-items: center; gap: 10px; width: 100%;">
                <input type="file" name="resume" accept=".pdf,.doc,.docx" required />
                <button type="submit" class="action-button"><i class="fas fa-upload"></i> Upload & Analyze</button>
            </form>
